...
public class FuncoesIniciais {
    ...
    /**
     * Funcao isPuzzle verifica se a string puzzle em construcao se comporta como um puzzle, so pode ter digitos de pistas ou pretas
     ...
     * @returns true se tiver tamanho rows*cols e se digitos nao sao repetidos e tem tamanho igual soma de n de pistas e pretas
     */
    ...
    public static boolean isRectangle(int rows, int cols, String spc, int digito, int posInicial) {
        ...
        while(posicao < spc.length() && rowOfPosition(posicao, cols) == posInicial && spc.charAt(posicao) == digitChar){
            largura++;
            posicao++;
        }
        ...
        for(int i = linhaInicial; i<linhaInicial+altura; i++){
            for(int j = colunaInicial; j < colunaInicial + largura; j++){
                ...
                if(spc.charAt(pos) != digitChar) return false;
            }
        }
        return true;
    }
    /**
     * Funcao isSolution verifica se string puzzle em construcao é solucao valida ou nao
